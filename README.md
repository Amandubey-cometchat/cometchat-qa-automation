...
| `after_campaign_failed`, `after_feed_item_interacted`, `after_push_notification_sent`/`delivered`/`clicked` (5) | `after_campaign_failed`: no reliable way found to trigger a genuine failure (a fake recipient still produces "completed"). `after_feed_item_interacted`: no REST endpoint exists (checked the documented surface plus several plausible undocumented paths, all 404) — likely SDK-only. The 3 push triggers need a real push channel with FCM/APNs credentials and a real device, neither of which exist on any of these apps. The other 5 Campaign triggers are automated — see `src/triggers/campaign/campaign.triggers.ts` | `src/registry/campaign.registry.ts` |
